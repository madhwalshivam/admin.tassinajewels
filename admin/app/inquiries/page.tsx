...
export default function InquiriesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] md:h-screen overflow-hidden font-light w-full" style={{ background: '#f8f5f0' }}>
      {/* Left Panel */}
      <div className="flex-1 flex flex-col p-4 md:p-8 overflow-hidden min-w-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-light uppercase tracking-wide" style={{ color: '#1B4332' }}>Inquiries</h1>
            <p className="text-[10px] md:text-xs text-gray-500 mt-1">Manage B2B wholesale quotation requests</p>
          </div>
          <a href="/api/export" className="w-full sm:w-auto px-4 py-3 sm:py-2.5 rounded-xl text-xs uppercase tracking-wider font-normal border transition-all hover:bg-white min-h-[44px] flex items-center justify-center shrink-0" style={{ borderColor: '#E3BA45', color: '#C9A82C' }}>
            Export CSV
          </a>
        </div>
        ...
        <div className="relative mb-4 shrink-0">
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-xs outline-none focus:border-yellow-400 font-light bg-white"
          />
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-4 bg-white rounded-2xl p-1 shadow-sm border border-gray-100 shrink-0 overflow-x-auto scrollbar-none">
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)}
              className="flex-1 min-w-[75px] py-2 rounded-xl text-[10px] uppercase tracking-widest font-normal transition-all"
              style={{
          ...
          {loading ? (
            ...
            <div key={inq.id} onClick={() => setSelected(inq)}
              ...
              <div className="flex items-start justify-between mb-1.5 gap-2">
                <span className="font-normal text-sm truncate" style={{ color: '#1B4332' }}>{inq.name}</span>
                <span className={`px-2.5 py-0.5 rounded-full text-[9px] uppercase tracking-wider font-semibold border shrink-0 ${statusColor[inq.status] || 'bg-gray-100 text-gray-600'}`}>
                  {inq.status}
                </span>
              </div>
              <div className="text-xs text-gray-500 font-light truncate">{inq.email}</div>
              {inq.product_title && (
                <div className="text-xs text-gray-400 mt-2 flex items-center gap-1.5 truncate">
                  ...
                  <span className="truncate">{inq.product_title}</span>
                </div>
              )}
      ...
      {/* Right Detail Panel */}
      {selected ? (
        <div className="fixed inset-0 z-40 bg-white md:static md:w-96 md:flex md:flex-col md:border-l md:border-gray-100 p-6 md:p-8 overflow-y-auto shrink-0 flex flex-col justify-between h-full">
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-normal text-base uppercase tracking-wider" style={{ color: '#1B4332' }}>Inquiry Detail</h2>
              <button onClick={() => setSelected(null)} className="w-8 h-8 rounded-full bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-gray-400 min-h-[32px] min-w-[32px]">
          ...
          <div className="mt-auto pt-4 border-t border-gray-100 bg-white sticky bottom-0">
            {/* Quick Actions */}
            <div className="space-y-2 mb-4">
              {selected.whatsapp && (
                <a
                  ...
                  target="_blank"
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-xs uppercase tracking-wider font-normal bg-emerald-600 text-white hover:bg-emerald-700 transition-all shadow-sm min-h-[44px]"
                >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-xs uppercase tracking-wider font-normal border transition-all hover:bg-gray-50 min-h-[44px]"
              ...
              <div className="grid grid-cols-2 gap-2">
                {['pending', 'replied', 'converted', 'cancelled'].map(s => (
                  <button key={s} onClick={() => updateStatus(selected.id, s)}
                    className={`py-2.5 rounded-xl text-[10px] uppercase tracking-wider font-normal transition-all min-h-[36px] ${selected.status === s ? 'opacity-40 cursor-default' : 'hover:opacity-80'}`}
                    style={{
            ...
            <button onClick={() => del(selected.id)} className="w-full py-3 rounded-xl text-xs uppercase tracking-wider font-normal bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 transition-all flex items-center justify-center gap-1.5 min-h-[44px]">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="hidden md:flex w-96 bg-white border-l border-gray-100 items-center justify-center shrink-0">
