  ...
  return (
    <div className="p-8 font-light max-w-6xl mx-auto">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-light uppercase tracking-wide" style={{ color: '#1B4332' }}>Dashboard</h1>
      ...
      {/* Quick Action Panel */}
      <div className="flex gap-2.5 mb-8 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 items-center justify-between">
        <div className="text-xs text-gray-500 uppercase tracking-wider font-medium">Quick Actions</div>
        <div className="flex gap-2">
          <Link href="/products" className="px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider font-normal transition-all flex items-center gap-1.5" style={{ background: '#1B4332', color: '#E3BA45' }}>
          ...
          <Link href="/inquiries" className="px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider font-normal border transition-all hover:bg-gray-50" style={{ borderColor: '#1B4332', color: '#1B4332' }}>
            View Inquiries
          </Link>
          <a href="/api/export" className="px-4 py-2.5 rounded-xl text-xs uppercase tracking-wider font-normal border transition-all hover:bg-gray-50" style={{ borderColor: '#E3BA45', color: '#C9A82C' }}>
            Export CSV
          </a>
        </div>
      </div>
            ...
            <tbody>
              {stats.recentInquiries.map((inq: any) => (
                <tr key={inq.id} className="border-b border-gray-50 hover:bg-gray-50 transition-all">
                  <td className="px-6 py-4 font-normal" style={{ color: '#1B4332' }}>{inq.name}</td>
                  <td className="px-6 py-4 text-gray-500">{inq.email}</td>
                  <td className="px-6 py-4 text-gray-600">{inq.product_title || '—'}</td>
                  <td className="px-6 py-4 text-gray-400 font-mono">{new Date(inq.created_at).toLocaleDateString()}</td>
